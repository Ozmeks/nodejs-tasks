...
import { getMessage, IMessage } from './helpers/locales.js';
import axios from 'axios';

const saveToken = async (token: string, m: IMessage) => {
	if (!token.length) {
...
const saveCity = async (city: string, m: IMessage) => {
	if (!city.length) {
		printError(m.noCity);
		return;
	}
...
const deleteCity = async (city: string, m: IMessage) => {
	if (!city.length) {
		printError(m.noCity);
		return;
	}
	try {
		const existing = await getKeyValue(TOKEN_DICTIONARY.city) ?? [];
    if (existing.includes(city)) {
      const newArray = existing.filter((el: string) => el !== city);
...
const saveLanguage = async (language: string, m: IMessage) => {
	if (!language.length) {
...
const getForecast = async (m: IMessage) => {
	try {
